...
// We keep this legacy stock endpoint behind the same shared guard as the
// current stock APIs, so expired sessions cannot update inventory directly.
...
$role = strtolower(trim((string) ($account['role'] ?? 'user')));

if (!in_array($role, ['admin', 'user', 'staff'], true)) {
