document.addEventListener('DOMContentLoaded', function () {
  ...
  function normalizeStoredStatus(movement, rawStatus) {
    var normalized = String(rawStatus || '').toLowerCase().trim();
    var labelMap = movement.movement_type === 'in' ? {
      'order dispatched': 'order_dispatched',
      'in transit': 'in_transit',
      'received at warehouse': 'received'
    } : {
      'dispatched from warehouse': 'dispatched',
      'stock received at hub': 'hub',
  ...
  function getStatusValue(movement) {
    return movement.movement_type === 'in'
      ? normalizeStoredStatus(movement, movement.incoming_status)
      : normalizeStoredStatus(movement, movement.movement_status);
  }

  function getStatusLabel(movement) {
    var statusValue = getStatusValue(movement);
    if (movement.movement_type === 'in') {
      return incomingStatusLabels[statusValue] || 'Order Dispatched';
    }

    return movementStatusLabels[statusValue] || 'Dispatched from Warehouse';
  }
  ...
  function getStatusOptionsMarkup(movement) {
    var activeStatus = getStatusValue(movement);
      ...
      '<div class="stock-history-select" data-history-select-root data-movement-id="' + movement.id + '">' +
        ...
        '<div class="stock-history-select__menu" data-history-select-menu hidden>' +
          options.map(function (option) {
            var isActive = option.value === activeStatus;
    ...
    historySelects.forEach(function (root) {
      ...
      root.querySelectorAll('[data-history-select-option]').forEach(function (optionButton) {
        optionButton.addEventListener('click', function () {
          ...
          fetch('api/stock/user_update_status.php', {
            method: 'POST',
            headers: {
  ...
  function loadProducts() {
    return fetch('api/products/get_user_products.php')
      .then(function (response) { return response.json(); })
      .then(function (products) {
        ...
        products.forEach(function (product) {
          ...
          option.setAttribute('data-stock', product.qty || 0);
          option.setAttribute('data-price', product.unit_price || 0);
  ...
  function loadHistory() {
    return fetch('api/stock/user_list.php')
      .then(function (response) { return response.json(); })
      .then(function (data) {
